...
# inducible <- read.table("Data/Motif_HOCO_Inducible_enhancer.profile", header=T, sep="\t")
# tag <- "enhancer"
inducible <- read.table("Data/Motif_HOCO_Inducible_promoter.profile", header = T, sep = "\t")
tag <- "promoter"
xlim <- c(0, 0.015)
ylim <- c(0, 4.5)
...
motif_analysis <- function(input, title, xlim, ylim) {
  ...
  col_scale <- timing / 12
  col_face <- rep(rgb(0.5, 0.5, 0.5, 0.0), length(tf.list)) # Color is set to transparent if TF not inducible
  ...
  plot(peak, fold,
    bg = col_face, col = col_edge, cex = increase, pch = 21, xlim = xlim, ylim = ylim,
...
pdf(paste("Figure/Motif_enrichment_and_TF_expression.", tag, ".pdf", sep = ""))
res_inducible <- motif_analysis(inducible, paste("Inducible", tag), xlim, ylim)
# Plot figure legend
...
pdf(paste("Figure/Motif_inducible_", tag, ".pdf", sep = ""))
par(mfrow = c(2, 2))
