...
################################################################
# 4.1 Control EP pairs
################################################################

# Now we decided to use 200kb as the cutoff
selected_eRNA <- names(index_eRNA)
selected_eRNA <- selected_eRNA[!(selected_eRNA %in% dee)]
selected_gene <- names(index_rpkm)
selected_gene <- selected_gene[!(selected_gene %in% deg)]
ctrl_e <- GRange_index(eRNA, selected_eRNA)
ctrl_g <- GRange_index(gene, selected_gene)
mat_e <- eRNA_rpkm[selected_eRNA, ]
mat_g <- rpkm[selected_gene, ]
# ctrl_PE <- find_pair(a=ctrl_g, b=ctrl_e, a_mat=mat_g, b_mat=mat_e, distance=c(0, 2*10^5), select_best = F)
#
# write.table(PE, "Data/PE.list", quote=F, row.names=F, col.names=F, sep="\t")
tp <- find_pair(a = ctrl_g, b = ctrl_e, a_mat = mat_g, b_mat = mat_e, distance = c(0, 2 * 10^5), select_best = F, output_symbol = F)
tp$symbol <- ref_2_symbol_local(tp[, 1])
tp$coord <- "chr0"
tp$enh_hg18 <- "chr0"
tp$enh_hg19 <- "chr0"
...
for (i in 1:nrow(tp)) {
  tp$coord[i] <- pair_region(tp[i, 1], tp[i, 2])
  tp_hg18 <- GRange_index(eRNA, tp[i, 2])
  temp <- liftOver_function(tp_hg18, ch = ch)
  if (!is.null(temp)) {
    tp_hg19 <- temp[1]
    tp$enh_hg19[i] <- paste(seqnames(tp_hg19), start(tp_hg19), end(tp_hg19))
  }
...
tp <- cbind(tp, rpkm[tp$id_a, ])
tp <- cbind(tp, eRNA_rpkm[tp$id_b, ])
write.csv(tp[, ], "Data/Control_EP.csv", row.names = F)
...
plot((c(1:nbin) - 0.5) * step / 10^6, fg,
  xlab = "Distance to Enhancers (MB)", ylab = "Normalized Target Occurence", main = "",
